function Fn_CreateExtractInfoHTML(filesdirectory,xmlfilename)
  
  fileExtractInfoHtml = fopen(sprintf('%s\\%s.html',filesdirectory,xmlfilename),'w');
  fprintf(fileExtractInfoHtml,'%s\n%s\n%s\n%s\n','<HEAD>','<TITLE> Extract Info Report: Name </TITLE>','</HEAD>','<BODY>');
  linkstr='<BR><h3>BlockNo.BlockType "<A href="matlab: hilite_system(''blckpath'');">blckname</A>"</h3>';
