function Fn_ExecuteATestCaseFromXML(modelcompltname,filesdirectory,testSuiteFilepath,dsrdTCNo,dsrdOutNo,SimTime,SimStep)
  ...
  [Output,~,OutSigTime]=Fn_ExecuteATestCase_Delphi(true,modelcompltname,testCase,ReportCov,InputTypesVar,externalinputname,...
    externaloutputname,SimTime,SimStep,InputTypesModeVar,IsSignedVal,WordLengthVal,SlopeVal,BiasVal,FractionLengthVal);
   
  if (dsrdOutNo==3) && (~isempty(strfind(modelcompltname,'17')))
    load('E:\mut17.mat');
    plot(ClutchDCScheduled.time,ClutchDCScheduled.signals.values,'LineWidth',3);
  else if (dsrdOutNo==3) && (~isempty(strfind(modelcompltname,'34')))
    load('E:\mut34.mat');
    plot(ClutchDCScheduled.time,ClutchDCScheduled.signals.values,'LineWidth',3);
  else
    plot(OutSigTime,Output(dsrdOutNo,:));
  end
  xlabel('Time');
  title(strrep(char(OutputNamesVar(dsrdOutNo)),'_','\_') );
